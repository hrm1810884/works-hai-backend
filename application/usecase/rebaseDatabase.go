...
func (usecase *RebaseDatabaseUsecase) DeleteImagesExceptCenter() (err error) {
	...
	if err != nil {
		return fmt.Errorf("failed to delete all the images: %w", err)
	}
...
func (usecase *RebaseDatabaseUsecase) OverrideInitialImage() (err error) {
	...
	fmt.Printf("data generated: %d", len(data));
	...
	if err != nil {
		...
		aiId := aiData.GetId()
		fmt.Printf("center image's filename: %v", aiId.GetDrawingName());
